...
def raise_error(e: str, error_type: int, stop: bool=True):
...
def main():
    ...
    with st.status("Marking Attendance...", expanded=True) as status:
        ...
        try:
            ...
            raise_error(f"Error extracting metadata: {e}", 4)
        ...
        try:
            ...
            raise_error(f"Error parsing main sheet: {e}", 4)
        ...
        try:
            ...
            raise_error(f"Error recording attendees: {e}", 5)
        ...
        try:
            ...
            raise_error(f"Error sorting missing students: {e}", 5)
        ...
        try:
            ...
            raise_error(f"Merge footer cells error: {e}", 5)
        ...
        try:
            ...
            try:
                ...
                raise_error(f"Error while identifying new rows: {e}", 5)
            else:
                print("ind is", ind)
            ...
            raise_error(f"Error formatting new cells: {e}", 5)
        ...
        try:
            ...
            for i in range(22):
                ...
                try:
                    ...
                    raise_error(f"Error while updating formulas: {e}", 5)
            ...
            raise_error(f"Error updating formulas : {e}", 5)
        ...
        for i, row in temp_dest.iterrows():
            for col in temp_dest.columns:
                ...
                try:
                    ...
                    print(i+2, col_idx, row[col])
                    raise_error(f"Error saving changes: {e}", 5, False)
                except Exception as e:
                    raise_error(f"Error saving changes: {e}", 5)
        ...
        st.session_state.completion_message = f"{attendees_num} people attended the {paper} class held on the {date.strftime("%dth of %B, year %Y")}"
        status.update(
...
with col2:
    csv_file = st.file_uploader("Upload an attendance list", type=["csv"], disabled=False)
